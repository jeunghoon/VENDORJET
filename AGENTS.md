...
## 전체 설계·진행 방향 (2025-11-30 기준 요약)
- 아키텍처: Flutter 클라이언트(`vendorjet/`) + Node/Express(TypeScript) API(`server/`) + SQLite 시드(향후 PostgreSQL로 확장) 구조.
- 멀티테넌시/권한: JWT + 멤버십(role)로 테넌트 전환 지원, Admin 전용 API 분리 필요.
- 도메인 범위: 상품/고객/주문 기본 CRUD + Buyer Portal 재주문/배송일 선택 흐름까지 구현. 재고/입고/회계/첨부는 미구현 상태.
- UX 키포인트: 반응형(M3), 대시보드 지표, 주문·상품·고객 필터/상태뷰, Buyer Portal의 단일 주문 화면 및 히스토리 불러오기.
- 운영/테스트: 시드 데이터 기반 로컬 실행, 자동 테스트·CI 없음, 보안(비밀번호 해시/토큰 갱신)과 RBAC 세분화 미비.

## 단기 우선 과제
1) **보안/권한 강화**: bcrypt 해시, 토큰 만료·리프레시, Admin 라우트 보호, 속도 제한 추가.  
2) **재고/LOT 도입**: `warehouses/locations/inventory_lots/stock_movements` 스키마 추가 후 주문 확정 시 가용 재고 차감 및 이벤트 로그 기록.  
3) **데이터 모델 확장**: 상품 바코드·규격·온도구분·HS 코드, 고객 신용/결제조건 필드 추가.  
4) **API 품질**: 페이지네이션·정렬 공통 파라미터, 에러 표준 `{data,error,meta}` 적용, 첨부 업로드 엔드포인트 설계.  
5) **클라이언트 정비**: 재고/입고 UI 신설, 주문·BuyerPortal에 LOT/재고 표시, 역할 기반 액션 가시성, 실제 API용 리포지토리 분리.  
6) **운영/테스트**: 마이그레이션 스크립트, 통합 테스트(주문→재고 차감), 위젯 테스트(주문 생성/재주문), Docker-compose 템플릿 추가.

## 기록 규칙 예시
- `2025-11-30` 보안 강화 설계 초안 작성, JWT 만료/리프레시 요구사항 합의.
- `2025-12-02` 재고 스키마 초안 PR 올림, 시드/마이그레이션 포함, API 연동 미완료(다음 작업으로 이월).
...
- [ ] 상용 서버 후보 비교/선택 기준 정리(로컬 서버 완성 후 착수)


## 작업 로그 (2025-12-02)
- 2025-12-02 로컬 서버 우선 전략 확정, Firebase 관련 계획/문서를 정리하고 향후 서버 선택은 로컬 완성 후 결정하기로 합의.
- 2025-12-02 Buyer Portal 안내/타이틀 정리 및 고객 정보 확장: 상품 목록이 비어 있을 때 승인 대기 문구를 거래처 명과 함께 표시하고, 메인 타이틀을 구매처 포털로 변경했으며 도매 고객 목록에 전화/주소를 추가 노출.

...
- 이 문서는 가이드 용도로 유지하며, **작업 로그는 `docs/backend/work_log.md`에 별도 기록**한다. 변경 시 날짜를 명시하고 *계획 → 실행 → 결과* 순으로 정리한다.
...
## 작업 로그 관리 지침
- `AGENTS.md`에는 가이드와 체크리스트만 유지하고, 작업 로그는 `docs/backend/work_log.md`에 날짜별로 기록합니다.
- 새 작업을 시작하면 `YYYY-MM-DD-번호` 형식으로 `docs/backend/work_log.md`에 계획 → 실행 → 결과를 간단히 남깁니다.
- 모든 문서는 UTF-8 인코딩을 유지합니다.
...
## 작업 로그 관리
- 작업 로그 분량을 줄이기 위해 별도 파일 `docs/backend/work_log.md`에 기록·보관한다.
- 기존 로그는 모두 해당 파일로 옮겼으며, 앞으로도 *계획 → 실행 → 결과* 형식을 유지한다.
- 새 로그는 UTF-8 한글로 작성하고 날짜 키를 `YYYY-MM-DD[-번호]` 형태로 명시한다.

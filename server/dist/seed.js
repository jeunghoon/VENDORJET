...
const db = new better_sqlite3_1.default(path_1.default.join(__dirname, '..', 'vendorjet.db'));
...
function seedTenantsUsersMemberships() {
    ...
    const insertTenant = db.prepare('INSERT OR REPLACE INTO tenants (id, name, locale, created_at, phone, address) VALUES (?,?,?,?,?,?)');
    tenants.forEach((t) => insertTenant.run(t.id, t.name, t.locale, nowIso, t.phone, t.address));
    const insertUser = db.prepare('INSERT OR REPLACE INTO users (id, email, password_hash, name, phone, user_type) VALUES (?,?,?,?,?,?)');
    const users = [
    ...
    const insertMembership = db.prepare('INSERT OR REPLACE INTO memberships (user_id, tenant_id, role, status) VALUES (?,?,?,?)');
...
function seedProducts() {
    const insertProduct = db.prepare('INSERT OR REPLACE INTO products (id, tenant_id, sku, name, price, variants_count, categories, tags, low_stock, image_url) VALUES (?,?,?,?,?,?,?,?,?,?)');
    const acmeProducts = [
    ...
    acmeProducts.forEach((p, idx) => insertProduct.run(`p_acm_${idx + 1}`, 't_acme', p.sku, p.name, p.price, 1, JSON.stringify(p.cat), JSON.stringify([]), p.low, null));
    const novaProducts = [
    ...
    novaProducts.forEach((p, idx) => insertProduct.run(`p_nov_${idx + 1}`, 't_nova', p.sku, p.name, p.price, 1, JSON.stringify(p.cat), JSON.stringify([]), p.low, null));
}
function seedCustomers() {
    const insertCustomer = db.prepare('INSERT OR REPLACE INTO customers (id, tenant_id, name, contact_name, email, tier, created_at, segment) VALUES (?,?,?,?,?,?,?,?)');
    const customers = [
    ...
    customers.forEach((c, idx) => insertCustomer.run(c.id, c.tenantId, c.name, c.contact, c.email, c.tier, new Date(Date.now() - (idx + 1) * 86400000).toISOString(), c.segment));
}
function nextOrderCode(date) {
    ...
    db.prepare(`INSERT INTO order_code_sequences (date, last_seq) VALUES (?, ?)
     ON CONFLICT(date) DO UPDATE SET last_seq = excluded.last_seq`).run(datePart, next);
...
function seedOrders() {
    ...
    const insertLine = db.prepare('INSERT INTO order_lines (order_id, product_id, product_name, quantity, unit_price) VALUES (?,?,?,?,?)');
    const insertEvent = db.prepare('INSERT INTO order_events (order_id, tenant_id, action, actor, note, created_at) VALUES (?,?,?,?,?,?)');
    for (let i = 0; i < 5; i++) {
        ...
        insertOrder.run(orderId, tenantId, code, tenantId === 't_acme' ? 'seller_portal' : 'buyer_portal', `Buyer ${i + 1}`, '010-1234-5678', i % 2 === 0 ? '메모 확인' : null, i % 2 === 0 ? 'pending' : 'confirmed', 100 + i * 10, 2 + i, createdAt.toISOString(), actor, createdAt.toISOString(), actor, 'Auto-generated seed order', createdAt.toISOString(), actor, new Date(createdAt.getTime() + 86400000).toISOString());

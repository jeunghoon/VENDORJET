...
function recordEvent(orderId, tenantId, action, actor, note) {
...
router.patch('/:id', (req, res) => {
    ...
    if (!existing)
        return res.status(404).json({ error: 'not found' });
    const existingLines = (0, client_1.mapRows)(client_1.db.prepare('SELECT * FROM order_lines WHERE order_id = ? ORDER BY id ASC').all(id));
    ...
    let normalizedLines;
    let newItemCount = existing.item_count;
    let newTotal = existing.total;
    if (Array.isArray(lines) && lines.length > 0) {
        normalizedLines = lines.map((it, idx) => ({
            id: idx + 1,
            order_id: id,
    ...
    client_1.db.prepare(`UPDATE orders
     ...
     WHERE id = ? AND tenant_id = ?`).run(status, buyerNote, updateNote, desiredDeliveryDate, newItemCount, newTotal, nowIso, actor, statusUpdatedAt, statusUpdatedBy, id, tenantId);
    if (normalizedLines) {
        const trxLines = client_1.db.transaction(() => {
    ...
    if (buyerNote)
        changeNotes.push('구매자 메모 변경');
    if (updateNote)
        changeNotes.push(updateNote);
...
function loadOrderWithLines(id, tenantId) {
    const orderRow = selectOrderStmt.get([id, tenantId]);
    if (!orderRow)
        return null;
    const lines = (0, client_1.mapRows)(selectLinesStmt.all(id));
    const events = (0, client_1.mapRows)(selectEventsStmt.all(id, tenantId));
